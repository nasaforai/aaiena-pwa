...
interface QRScanScreenProps {
  ...
  onBack: () => void;
  onContinue: () => void;
  onBackToPhysical: () => void;
}

export default function QRScanScreen({ title, onBack, onContinue, onBackToPhysical }: QRScanScreenProps) {
  const copyCode = () => {
    ...
    <div className="bg-white flex max-w-[480px] w-full flex-col overflow-hidden mx-auto min-h-screen">
      ...
      <div className="flex-1 px-6 py-8">
        ...
        <div className="bg-gray-50 rounded-2xl p-6 mb-6">
          ...
          <p className="text-sm text-gray-600 text-center mb-4">Use your phone camera to scan</p>
        ...
        {/* Code Section */}
        <div className="mb-8">
          <p className="text-sm text-gray-600 text-center mb-4">Copy The Code To Continue In Kiosk</p>
          <div className="bg-purple-100 rounded-xl p-4 flex items-center justify-between">
            ...
            <button onClick={copyCode} className="p-2 hover:bg-purple-200 rounded-lg transition-colors">
              <Copy className="w-4 h-4" />
            </button>
          </div>
          <p className="text-xs text-gray-500 text-center mt-2">Valid Till One Day</p>
        </div>
